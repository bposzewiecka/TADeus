...
class TrackPlot(object):
    ...
    def draw_track(self, cols, chrom, start, end, interval_start, interval_end, name_filter):
        ...
        self.label_axis = axisartist.Subplot(fig, grids[0, 1])
        ...
        fig.set_size_inches(cm2inch(self.fig_width, self.get_height()))
        return fig
...
class PlotBed(TrackPlot):
    ...
    def set_colormap(self):
        ...
        if self.colormap and not self.min_value is None and not self.max_value is None:
    ...
    def draw(self,  chrom, start, end, width):
        ...
        for region in self.interval_tree:
            ...
            if entry.itemRGB is not None:
                rgb = '#' + entry.itemRGB
            elif self.colormap and entry.score:
        ...
        self.label_axis.text(0.15, 1, self.title, fontsize=8,
                      horizontalalignment='left',
                      verticalalignment='top', transform=self.label_axis.transAxes)
    ...
    def draw_gene_with_introns_flybase_style(self, entry, ypos, rgb, edgecolor):
        ...
        for idx in range(0, entry.block_count):
            ...
            if x0 < entry.thick_start < x1:
                positions.append((x0, entry.thick_start, 'UTR'))
                positions.append((entry.thick_start, x1, 'coding'))

            elif x0 < entry.thick_end < x1:
                positions.append((x0, entry.thick_end, 'coding'))
                positions.append((entry.thick_end, x1, 'UTR'))
        ...
        vertices = self._draw_arrow(self.axis, first_pos[0], first_pos[1], entry.strand, ypos)
...
class PlotBedGraph(TrackPlot):
    ...
    def draw(self, chrom, start, end, width):
        ...
        self.label_axis.text(0.15, 1, self.title,
    ...
    def draw(self, chrom, start, end, width):
        ...
        if self.x_labels:
            ...
            labels = ["{:.2f}".format((x / 1e6))
                      for x in ticks]
            labels[-1] += "Mbp"
            self.axis.get_xaxis().set_tick_params(
                which='both',
                bottom='on',
                top='off',
                direction='out')
        ...
        try:
            if self.transform and self.transform in ['log', 'log1p']:
